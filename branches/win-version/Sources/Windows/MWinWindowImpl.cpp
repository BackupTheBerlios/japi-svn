...
{
	AddHandler(WM_DESTROY,		&MWinWindowImpl::WMDestroy);
	AddHandler(WM_CHAR,			&MWinWindowImpl::WMChar);
	AddHandler(WM_KEYDOWN,		&MWinWindowImpl::WMKeydown);
	AddHandler(WM_SYSKEYDOWN,	&MWinWindowImpl::WMKeydown);
}
...
void MWinWindowImpl::SetHandle(HWND inHandle)
{
	if (mHandle != inHandle)
	{
		if (mHandle != nil)
...
MWinWindowImpl* MWinWindowImpl::FetchWindow(HWND inHandle)
{
	return reinterpret_cast<MWinWindowImpl*>(::GetPropW(inHandle, L"m_window_imp"));
}

void MWinWindowImpl::Create(MRect inBounds, string inTitle)
{
	DWORD		exStyle = 0;
	...
	lWndClass.lpszClassName = L"MWinWindowImp";

	if (not ::GetClassInfoExW(instance, lWndClass.lpszClassName, &lWndClass))
	{
		DWORD style = CS_OWNDC | CS_VREDRAW | CS_HREDRAW;

		lWndClass.style = style;
		lWndClass.lpfnWndProc = &MWinWindowImpl::WinProcCallBack;
		//lWndClass.cbClsExtra = cbClsExtra;
		//lWndClass.cbWndExtra = cbWndExtra;
		lWndClass.hInstance = instance;
		lWndClass.hIcon = ::LoadIconA(instance, "doc-icon-resource");
		lWndClass.hCursor = ::LoadCursor(0, IDC_ARROW);
		lWndClass.hbrBackground = (HBRUSH)(COLOR_WINDOW);
		//lWndClass.lpszMenuName = menuName;
		//lWndClass.lpszClassName = L"MWinWindowImp";
		lWndClass.hIconSm = ::LoadIconA(instance, "small-doc-icon-resource");
...
bool MWinWindowImpl::WMChar(HWND inHWnd, UINT inUMsg, WPARAM inWParam, LPARAM inLParam, int& outResult)
{
	bool result = false;
...
bool MWinWindowImpl::WMKeydown(HWND inHWnd, UINT inUMsg, WPARAM inWParam, LPARAM inLParam, int& outResult)
{
	//HKeyDown keyDown;
	//
	//keyDown.text = "";
...
int	MWinWindowImpl::WinProc(HWND inHandle, UINT inMsg, WPARAM inWParam, LPARAM inLParam)
{
	int result = 0;
	...
	{
		if (mOldWinProc != nil and mOldWinProc != &MWinWindowImpl::WinProcCallBack)
...
int MWinWindowImpl::DefProc(HWND inHWnd, UINT inUMsg, WPARAM inWParam, LPARAM inLParam)
{
	return ::DefWindowProcW(inHWnd, inUMsg, inWParam, inLParam);
}

LRESULT CALLBACK MWinWindowImpl::WinProcCallBack(HWND hwnd, UINT uMsg,
...
//
	try
	{
		if (uMsg == WM_CREATE)
		{
			...
			MWinWindowImpl* impl = reinterpret_cast<MWinWindowImpl*>(cs->lpCreateParams);
			impl->SetHandle(hwnd);
			result = 0;
		}
		else
		{
			MWinWindowImpl* impl = MWinWindowImpl::FetchWindow(hwnd);
