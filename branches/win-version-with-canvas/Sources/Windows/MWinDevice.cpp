	...
	, mDpiScaleY(1)
{
	...
	MWindow* window = inCanvas->GetWindow();
	THROW_IF_NIL(window);

	MWinCanvasImpl* canvasImpl = dynamic_cast<MWinCanvasImpl*>(inCanvas->GetImpl());

	mRenderTarget = canvasImpl->GetRenderTarget(sD2DFactory);

	int32 x = 0, y = 0;
	inCanvas->ConvertToWindow(x, y);
...
MDeviceImp* MDeviceImp::Create(
	...
	bool				inCreateOffscreen)
{
	return new MWinDeviceImpl(inCanvas, inRect, inCreateOffscreen);
}
